...
class Dataset:
    ...
    def _scan_directory(self, partition_index, total_partitions) -> Iterator[Sample]:
        ...
        if total_partitions > 1:
            ...
            end_index = start_index + partition_size if partition_index < total_partitions - 1 else len(file_list)

            file_list = file_list[start_index:end_index]

        for file_path in file_list:
            yield Sample(full_path=file_path, relative_path=os.path.relpath(file_path, start=self.root_dir))
