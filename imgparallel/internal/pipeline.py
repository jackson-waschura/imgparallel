...
from imgparallel.internal.file_io import read_image_from_path, write_image_to_path, check_permissions
    ...
    def run(self, num_processes_per_stage=1, verbose: bool = False, dry_run: bool = False):
        ...
        for i, stage in enumerate(self.stages):
            for p_idx in range(num_processes_per_stage):
                ...
                p = mp.Process(
                    target=worker,
                    args=(in_queue, out_queue, stage, p_idx, num_processes_per_stage)
                )
                p.start()
                processes.append(p)
